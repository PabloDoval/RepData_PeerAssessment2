...
In order to come to a more usable data set and to get rid of some of the data quality issues, the dimensionality of the EventType column will be reduced via gruping of similar events. In order to select the groups, the NOAA document [Storm Data Preparation](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf) has been used as a reference; in particular, the table 2.1.1 (Storm Data Event Table) and the **Directive 10-1605**, which limits the standard events to a list of 48 approved ones:
...
ggplot(healthAnalysis, aes(x = EventType, y = Total, fill = Outcome)) + 
  ...
  ggtitle("Top Events by Health Impact") +
